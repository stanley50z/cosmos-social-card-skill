...
- Set `layout` starting positions to match the chosen 范式 (e.g. 大字报型 → 主文字 large & high; 极简摄影型 → 主文字 压在留白处、字号收敛). Keep text between the 150px safe-zone lines.
- **If the user gave you the actual cover photo** (pasted into chat), read its composition and pre-set the rest from what you see — this is the "AI 先自己操作" pass:
  ...
  - **明暗 → 文字 color + `scrim`**: dark area → 白字, no/low scrim; bright or busy area → set `scrim.on` with `mode` (`top`/`bottom`/`both`/`full`) and `opacity` over the text band so the text stays legible. Match 文字 color to the brand theme.
  - **构图被裁 → `bg` scale/x/y**: `object-fit:cover` center-crops to 900×1200; if the subject would be cut off or too small, pre-set `bg.scale`/`x`/`y` so the subject lands inside the safe zone.
